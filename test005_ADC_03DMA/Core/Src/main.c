...
union //share memory
{
	...
	int v2[20];
	long v3[10];
} buf; //using buffer memory preparation
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
	...
	if (++Mode > 5)
		Mode = 1;
	//printf("\033[2J")// Terminal screen clear
	cls();
}
/* USER CODE END 0 */
...
int main(void) {
	...
	while (1) {
		...
		case 1:
			...
			printf("\033[?25l\033[4;0HP(%d,%d,%d) : %d         \n", buf.v1[0],
					buf.v1[1], z, buf.v1[2]);
		...
		/*
		 ...
		 for(int i = 0;i<20;i++)//ROW index
		 {
		 for(int j = 0;j<10;j++)// COULUMN index
		 {
		 printf("%04X ",buf.v1[j]);
		 if(j == 5) printf("  ");
		 }
