...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc){
	...
	//HAL_ADC_Start_IT(&hadc1);//only once work.
}
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  //HAL_ADC_Start(&hadc1);
	  	...
	  	v = (double)(x/100) *2;
	  	//int v = ((double)(x * 3) / 4096) - 2;
	  	int xx = cx + v;
	  	xx = (xx > 78) ? 78 : (xx < 0) ? 0 :xx;
	  	...
	  	//v = ((double)(y * 3) / 4096) - 2;
	  	int yy = cy + w;
	  	yy = (yy > 23) ? 23 : (yy < 0) ? 0 : yy;
	  	//HAL_ADC_Start(&hadc1);
	  	//HAL_ADC_PollForConversion(&hadc1, 1000); // start Trigger
	  	int z = HAL_GPIO_ReadPin(Z_Axis_GPIO_Port, Z_Axis_Pin);
	  	printf("\033[0;0HADC Value : (%d,%d,%d)", xx, yy, z);  // location info
	  	printf("\033[%d;%dH \033[%d;%dH@\033[A\n", cy, cx, yy, xx);
	  	cx = xx; cy = yy;  //save current pos
	  	HAL_Delay(100);
